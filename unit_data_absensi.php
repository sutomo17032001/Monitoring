...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Kepala Unit</h1>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered" id="dataTable1" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        <?php 
                                        $sql = "SELECT p.nama_pegawai, p.jabatan_pegawai, u.nama_unit, a.latitude, a.longitude, a.nama_device, a._timestamp FROM pegawai p JOIN absensi a ON p.id_pegawai = a.id_pegawai JOIN unit u ON u.Id_unit = p.id_unit WHERE  u.id_unit ='$unit' AND jabatan_pegawai = 'kepala unit' group by p.nama_pegawai ORDER BY a._timestamp";
                                        ...
                                        $sql5 = "SELECT COUNT(_timestamp) AS A FROM absensi a JOIN pegawai p ON p.id_pegawai = a.id_pegawai WHERE MONTH(a._timestamp) = MONTH(CURRENT_DATE) AND p.id_pegawai = '$id' AND a.terlambat = 1 ";
                                        $query5 = mysqli_query($conn, $sql5);
                                        $result5 = mysqli_fetch_assoc($query5);

                                        $sql3 = "SELECT COUNT(*) AS y FROM cuti c JOIN pegawai p ON c.id_pegawai = p.id_pegawai WHERE MONTH(c._time) = MONTH(CURRENT_DATE) AND c.id_pegawai = '$id' AND c.id_katagoricuti = 1 ";
                                        $query3 = mysqli_query($conn, $sql3);
                                        $result3 = mysqli_fetch_assoc($query3);

                                        //ambil data cuti
                                        $sql4 = "SELECT COUNT(*) AS z FROM cuti c JOIN pegawai p ON c.id_pegawai = p.id_pegawai WHERE MONTH(c._time) = MONTH(CURRENT_DATE) AND c.id_pegawai = '$id' AND not c.id_katagoricuti = 1 ";
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" id="dataTable2" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        <?php 
                                        include 'config.php';
                                        $sql0 = "SELECT id_unit FROM pegawai WHERE id_pegawai = '$id' ";
                                        $query0 = mysqli_query($conn, $sql0);
                                        $result0 = mysqli_fetch_assoc($query0);

                                        $sql = "SELECT p.nama_pegawai, p.id_pegawai FROM pegawai p JOIN absensi a ON p.id_pegawai = a.id_pegawai JOIN unit u ON u.Id_unit = p.id_unit WHERE  u.id_unit = '".$result0['id_unit']."' AND p.jabatan_pegawai = 'Karyawan' group by p.nama_pegawai order by a._timestamp";
                                        $query = mysqli_query($conn, $sql);
                                        ...
                                        while($row = mysqli_fetch_array($query)){   
                                        $sql1 = "SELECT COUNT(_timestamp) AS x FROM absensi a JOIN pegawai p ON p.id_pegawai = a.id_pegawai WHERE MONTH(a._timestamp) = MONTH(CURRENT_DATE) AND p.id_pegawai = '".$row['id_pegawai']."' ";
                                        $query1 = mysqli_query($conn, $sql1);
                                        $result1 = mysqli_fetch_assoc($query1);

                                        $sql3 = "SELECT COUNT(*) AS y FROM cuti c JOIN pegawai p ON c.id_pegawai = p.id_pegawai WHERE MONTH(c._time) = MONTH(CURRENT_DATE) AND c.id_katagoricuti = 1 and c.id_pegawai = '".$row['id_pegawai']."' ";
                                        $query3 = mysqli_query($conn, $sql3);
                                        $result3 = mysqli_fetch_assoc($query3);

                                        //ambil data cuti
                                        $sql4 = "SELECT COUNT(*) AS z FROM cuti c JOIN pegawai p ON c.id_pegawai = p.id_pegawai WHERE MONTH(c._time) = MONTH(CURRENT_DATE) AND c.id_katagoricuti = 1 and c.id_pegawai = '".$row['id_pegawai']."' ";
                                        $query4 = mysqli_query($conn, $sql4);
                                        $result4 = mysqli_fetch_assoc($query4);

                                        $sql5 = "SELECT COUNT(_timestamp) AS A FROM absensi a JOIN pegawai p ON p.id_pegawai = a.id_pegawai WHERE MONTH(a._timestamp) = MONTH(CURRENT_DATE) AND p.id_pegawai = '".$row['id_pegawai']."' AND a.terlambat = 1 ";
                                        $query5 = mysqli_query($conn, $sql5);
                                        $result5 = mysqli_fetch_assoc($query5);
                                        $bolos = $result2["b"] - $result1["x"];
                                        echo "
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            //highlight column table 2
            $('#dataTable2 tbody')
            .on( 'mouseenter', 'td', function () {
            var colIdx2 = table2.cell(this).index().column;
 
            $( table2.cells().nodes() ).removeClass( 'highlight' );
            $( table2.column( colIdx2 ).nodes() ).addClass( 'highlight' );
            }); 
            ...
            $('#dataTable1 tbody').on('click', 'td.detail-control', function () {
                ...
                window.location.href = "unit_detail_absensi.php";

            } );

            //open detail table 2
            $('#dataTable2 tbody').on('click', 'td.detail-control', function () {
